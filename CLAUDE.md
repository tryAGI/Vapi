...
- `src/libs/Vapi/generate.sh` -- Downloads spec, applies 8 fixes, runs autosdk with `--security-scheme`
...
The `generate.sh` applies 8 fixes to the downloaded spec via `jq`, plus `--security-scheme Http:Header:Bearer` for auth:

1. **Transcript enum:** Renames `transcript[transcriptType="final"]` to `transcript-final` (embedded quotes break C# string escaping)
2. **LMNTVoice language:** Flattens `oneOf` to simple `string` (oneOf with two enum types produces invalid generic type)
3. **Operator enums:** Replaces symbols (`=`, `!=`, `>`, `>=`, `<`, `<=`) with descriptive names (`eq`, `neq`, `gt`, `gte`, `lt`, `lte`) to avoid duplicate C# identifiers
4. **Punctuation enums:** Replaces punctuation characters (`!`, `)`, `,`, `:`, `;`, `?`, `|`, `||`, CJK/Devanagari/Arabic) with descriptive names
5. **Missing discriminator mappings:** Adds `code` -> `CreateCodeToolDTO` and `ghl` -> `GhlTool` to tool endpoint discriminators
6. **11labs rename:** Renames `11labs` discriminator key to `elevenlabs` (numeric prefix causes C# naming collisions)
7. **Enum deduplication:** Deduplicates string enum arrays to avoid CS0102 errors
8. **Duplicate parameter rename:** Renames second `name` query param in `/session` GET to `customerName` (two params named `name` — session name and customer name — cause CS0100)
